...
class SlackTutorial
  ...
  def self.welcome_text
    "Welcome to the SAE School Parents Slack orgainzation!\nHere are some basic features."
  end
  ...
  # in the hash shown above. When a user completes an action on the
  ...
  def self.update_item(team_id, user_id, item_index)
    # Update the tutorial section by replacing the empty checkbox with green
...
class API < Sinatra::Base
  ...
  post '/events' do
    # Extract the Event payload form the request and parse the JSON
    ...
    # URL's authenticity by sending a challenge token to y0our endpoint, expecting your app to echo it back.
...
class Events
  # You may notice that user and chanel IDs may be found in
  ...
  def self.message(team_id, event_data)
    user_id = event_data['user']
    # Don't process messages sent form our bot user
    unless user_id == $teams[team_id][:bot_user_id]

      # This is where our `message` event handlers go:

      # SHARED MESSAGE EVENT
      # To check for shared messages, we must chec for the `attachments` attribute
  ...
  def self.send_response(team_id, user_id, channel = user_id, ts = nil)
    ...
    if ts
      ...
      $teams[team_id]['client'].chat_postMessage(
        as_user: 'true',
        channel: channel,
        text: S?lackTutorial.welcome_text,
        attachments: $teams[team_id][user_id][:tutorial_content]
      )
    end
  end
